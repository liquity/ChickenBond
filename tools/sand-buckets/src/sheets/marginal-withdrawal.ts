import { StableSwapPool, xyFromDydx } from "../pool";

const ROWS = 1000;
const As = [10, 20, 50, 100, 200, 500, 1000, 2000, 5000];
const lpV = 1;
const D = 1e9; // Make it big, so that our withdrawal of 1 is tiny in comparison
const fee = 0.0004;
const adminFee = 0.5;

console.log(["dx/dy", ...As.map(A => `A=${A}`)].join(","));

for (let i = 1; i < ROWS; ++i) {
  const dxdy = i > ROWS / 2 ? 1 + ROWS * (fee / 80) + (i - ROWS / 2) : 1 + i * (fee / 40);

  const profits = As.map(A => {
    const [y, x] = xyFromDydx(A, D)(dxdy);
    const p = new StableSwapPool({ n: 2, A, fee, adminFee, balances: [x, y] });
    const lp = lpV / p.virtualPrice;
    const dx = p.removeLiquidityOneCoin(lp, 0);
    return dx - lpV;
  });

  console.log([dxdy, ...profits].join(","));
}
