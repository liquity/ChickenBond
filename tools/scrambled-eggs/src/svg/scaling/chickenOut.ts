...
export const chickenOutAnimations = (tokenID: number, s: number) => /*css*/ `
  ...
  #co-chicken-${tokenID} .co-left-leg path {
    ...
    transform-origin: ${scaleX(s, 420)}px ${scaleY(s, 525)}px;
  }

  #co-chicken-${tokenID} .co-right-leg path {
    animation: co-right-leg 0.3s infinite ease-in-out alternate;
    transform-origin: ${scaleX(s, 320)}px ${scaleY(s, 525)}px;
  }

  #co-chicken-${tokenID} .co-shadow {
    animation: co-shadow 0.3s infinite ease-in-out alternate;
    transform-origin: ${scaleX(s, 375)}px ${scaleY(s, 636)}px;
  }

  @keyframes co-run {
    10% { transform: translateY(0); }
    100% { transform: translateY(${round(s * 88)}px); }
  }

  @keyframes co-left-leg {
    20% { transform: rotate(-0deg); }
    100% { transform: rotate(-75deg) translateX(${round(s * 120)}px); }
  }

  @keyframes co-right-leg {
    0% { transform: rotate(5deg); }
    20% { transform: rotate(5deg); }
    100% { transform: rotate(70deg) translateX(${round(s * -120)}px); }
  }
...
const scaleCircleCoords = (s: number, cx: number, cy: number, r: number) => ({
  cx: scaleX(s, cx),
  cy: scaleY(s, cy),
  r: round(s * r),
...
export const chickenOutShadow = (s: number) => /*svg*/ `
...
export const chickenOutLeftLeg = (fill: string, s: number) => {
  const p = [
  ...
  return /*svg*/ `
    <path style="fill: ${fill}" d="${scalePath(s, p[0])}"/>
    <path style="fill: ${fill}" d="${scalePath(s, p[1])}"/>
  `;
};

export const chickenOutRightLeg = (fill: string, s: number) => {
  const p = [
...
export const chickenOutBeak = (fill: string, s: number) => {
  const p =
  ...
  return /*svg*/ `
    <path style="fill: ${fill}" d="${scalePath(s, p)}"/>
  `;
};

export const chickenOutChicken = (style: string, s: number) => {
  const p = [
  ...
  return /*svg*/ `
    <path style="${style}" d="${scalePath(s, p[0])}"/>
    <path style="fill: #000; mix-blend-mode: soft-light" d="${scalePath(s, p[1])}"/>
    <path style="fill: #000; mix-blend-mode: soft-light" d="${scalePath(s, p[2])}"/>
  `;
};

export const chickenOutEye = (s: number) => /*svg*/ `
  <circle style="fill: #fff" ${scaleCircleCoords(s, 434.63, 395.88, 8.32)}/>
  <circle style="fill: #000" ${scaleCircleCoords(s, 434.64, 395.88, 5.63)}/>
`;

export const chickenOutShell = (style: string, s: number) => {
  const p = [
