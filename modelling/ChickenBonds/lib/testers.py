...
class TesterSimpleToll(TesterInterface):
    ...
    def get_arbitrageurs(self, chicken, chicks):
        return list(filter(lambda chick: not chick.rebonder and not chick.lp, chicks))

    def get_arbitrageurs_with_stoken(self, chicken, chicks, threshold=0):
        return list(filter(lambda chick: not chick.rebonder and not chick.lp and chicken.stoken.balance_of(chick.account) > threshold, chicks))
    ...
    def get_stoken_spot_price(self, chicken):
        return chicken.stoken_amm.get_token_B_price()
    ...
    def get_stoken_apr_spot(self, chicken, data, iteration):
        ...
        previous_spot_price = data["stoken_price"][iteration-1]
        ...
        return (current_spot_price / previous_spot_price - 1) * TIME_UNITS_PER_YEAR
    ...
    def is_bootstrap_phase(self, iteration):
        return iteration <= BOOTSTRAP_ITERATION
    ...
    def buy_stoken(self, chicken, chicks, stoken_spot_price, fair_price):
        # print(f"\n --> Buying sTOKEN")
        # print(chicken.stoken_amm)

        # prevent crazy price increases in early stage
        target_price = min(fair_price, stoken_spot_price * 1.5)
        token_sell_amount = chicken.stoken_amm.get_input_A_amount_from_target_price_B(target_price)

        total_bought = 0
        remaining_amount = token_sell_amount
        for chick in self.get_arbitrageurs(chicken, chicks):
            # swap
            ...
            """
            print(f"swap_amount:          {swap_amount:,.2f}")
            print(f"bought_stoken_amount: {bought_stoken_amount:,.2f}")
            print(f"total_bought:         {total_bought:,.2f}")
            print(f"remaining_amount:     {remaining_amount:,.2f}")
            """
    ...
    def sell_stoken(self, chicken, chicks, stoken_spot_price, fair_price):
        # print(f"\n --> Selling sTOKEN")
        ...
        for chick in self.get_arbitrageurs_with_stoken(chicken, chicks, threshold=10):
            # swap
    ...
    def amm_arbitrage(self, chicken, chicks, iteration):
        if self.is_bootstrap_phase(iteration):
            return
        ...
        #print(f"price threshold:  {stoken_spot_price * (1 - self.amm_arbitrage_divergence):,.2f}")
        #print(f"price threshold:  {stoken_spot_price * (1 + self.amm_arbitrage_divergence):,.2f}")
        """
        ...
        if fair_price < stoken_spot_price * (1 - self.amm_arbitrage_divergence):
            self.sell_stoken(chicken, chicks, stoken_spot_price, fair_price)
        elif fair_price > stoken_spot_price * (1 + self.amm_arbitrage_divergence):
            self.buy_stoken(chicken, chicks, stoken_spot_price, fair_price)

        """
        print("After:")
        print(f"fair price:       {self.get_fair_price(chicken):,.2f}")
        print(f"stoken price:     {self.get_stoken_spot_price(chicken):,.2f}")
        """
    ...
    def redemption_arbitrage(self, chicken, chicks, iteration):
        ...
        if pol_ratio > stoken_spot_price * (1 + self.redemption_arbitrage_divergence):
            ...
            print(f"Backing ratio:   {pol_ratio:,.2f}")
            print(f"Price:           {stoken_spot_price:,.2f}")
            #print(f"Price threshold: {stoken_spot_price * (1 + self.redemption_arbitrage_divergence):,.2f}")
            #print(chicken.stoken_amm)
            #print(f"slippage amt: {chicken.stoken_amm.get_input_A_for_max_slippage(self.max_slippage, 0, 0):,.2f}")
            #print(f"target amt:   {chicken.stoken_amm.get_input_A_amount_from_target_price_B(pol_ratio):,.2f}")
            """
            amount_for_max_slippage = chicken.stoken_amm.get_input_A_for_max_slippage(self.max_slippage, 0, 0)
            amount_for_target_price = chicken.stoken_amm.get_input_A_amount_from_target_price_B(pol_ratio)
            if amount_for_target_price > amount_for_max_slippage:
                # this way is still profitable, in spite of large slippage
                token_swap_amount = amount_for_target_price / 2
            else:
                token_swap_amount = amount_for_target_price
