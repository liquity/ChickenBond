...
def newton_y(ANN: uint256, gamma: uint256, x: uint256[N_COINS], D: uint256, i: uint256) -> uint256:
    ...
    #log LogUint("convergence_limit", convergence_limit)

    __g1k0: uint256 = gamma + 10**18
    #log LogUint("__g1k0", __g1k0)
    ...
    for j in range(255):
        ...
        #log LogUint("_g1k0", _g1k0)

        # D / (A * N**N) * _g1k0**2 / gamma**2
        mul1: uint256 = unsafe_div(unsafe_div(unsafe_div(10**18 * D, gamma) * _g1k0, gamma) * _g1k0 * A_MULTIPLIER, ANN)
        #log LogUint("mul1", mul1)

        # 2*K0 / _g1k0
        mul2: uint256 = unsafe_div(10**18 + (2 * 10**18) * K0, _g1k0)
        #log LogUint("mul2", mul2)
        ...
        #log LogUint("fprime", fprime)

        # y -= f / f_prime;  y = (y * fprime - f) / fprime
        # y = (yfprime + 10**18 * D - 10**18 * S) // fprime + mul1 // fprime * (10**18 - K0) // K0
        y_minus: uint256 = mul1 / fprime
        #log LogUint("y_minus", y_minus)
        y_plus: uint256 = (yfprime + 10**18 * D) / fprime + y_minus * 10**18 / K0
        #log LogUint("y_plus", y_plus)
...
def get_xcp(D: uint256) -> uint256:
    ...
    #log LogUint("price_scale", self.price_scale)
    #log LogUint("x[0]", x[0])
    #log LogUint("x[1]", x[1])
...
def tweak_price(A_gamma: uint256[2],_xp: uint256[N_COINS], p_i: uint256, new_D: uint256):
    ...
    if last_prices_timestamp < block.timestamp:
        ...
        #log LogUint("price_oracle", price_oracle)
        self.last_prices_timestamp = block.timestamp
    ...
    if new_D == 0:
        ...
        #log LogUint("_xp[0]", _xp[0])
        #log LogUint("_xp[1]", _xp[1])
        #log LogUint("D_unadjusted", D_unadjusted)
    ...
    if old_virtual_price > 0:
        ...
        #log LogUint("xcp", xcp)
        #log LogUint("total_supply", total_supply)
        xcp_profit = old_xcp_profit * virtual_price / old_virtual_price

        t: uint256 = self.future_A_gamma_time
        #log LogUint("virtual_price", virtual_price)
        #log LogUint("old_virtual_price", old_virtual_price)
...
def _exchange(sender: address, i: uint256, j: uint256, dx: uint256, min_dy: uint256,
    ...
    #log LogUint("xp[0]", xp[0])
    #log LogUint("xp[1]", xp[1])
    #log LogUint("dx", dx)
    p: uint256 = 0
    dy: uint256 = 0
    ...
    n: uint256 = self.newton_y(A_gamma[0], A_gamma[1], xp, self.D, j)
    dy = xp[j] - n
    #log LogUint("xp[j]", xp[j])
    #log LogUint("n", n)
    #log LogUint("dy", dy)
    ...
    #log LogUint("dy", dy)
    #log LogUint("fee", self._fee(xp))
    #log LogUint("sub dy", self._fee(xp) * dy / 10**10)
...
def get_dy(i: uint256, j: uint256, dx: uint256) -> uint256:
    ...
    #log LogUint("x 0", xp[0])
    #log LogUint("x 1", xp[1])

    y: uint256 = self.newton_y(A_gamma[0], A_gamma[1], xp, D, j)
    #log LogUint("newton y", y)
    dy: uint256 = xp[j] - y - 1
    xp[j] = y
    if j > 0:
        dy = dy * PRECISION / price_scale
...
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256, receiver: address = msg.sender) -> uint256:
    ...
    #log LogUint("token_supply", token_supply)
    #log LogUint("D", D)
    #log LogUint("old D", old_D)
    #log LogUint("d_token", d_token)
...
@external
@view
def newton_y_wrapper(ANN: uint256, gamma: uint256, x: uint256[N_COINS], D: uint256, i: uint256) -> uint256:
    return self.newton_y(ANN, gamma, x, D, i)

@external
@view
def xp_wrapper() -> uint256[N_COINS]:
    return self.xp()

@external
def tweak_price_wrapper(A_gamma: uint256[2],_xp: uint256[N_COINS], p_i: uint256, new_D: uint256):
