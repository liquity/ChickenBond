...
contract ChickenBondManager is Ownable, ChickenMath, IChickenBondManager {
    ...
    */
    ...
    function chickenIn(uint256 _bondID) external {
        ...
        */
        ...
        uint256 accruedSLUSD = _calcAccruedSLUSD(bond.startTime, bondAmountMinusChickenInFee, backingRatio, updatedAccrualParameter);
        ...
        // Transfer the chicken in fee to the LUSD/sLUSD AMM LP Rewards staking contract during normal mode.
        if (!migration) {
    ...
    * In practice, the total acquired LUSD calculation will depend on the specifics of how Yearn vaults calculate
    ...
    function calcAccruedSLUSD(uint256 _bondID) external view returns (uint256) {
        ...
        return _calcAccruedSLUSD(bond.startTime, _getBondAmountMinusChickenInFee(bond.lusdAmount), calcSystemBackingRatio(), updatedAccrualParameter);
    }
    ...
    function calcBondSLUSDCap(uint256 _bondID) external view returns (uint256) {
        ...
        return _calcBondSLUSDCap(_getBondAmountMinusChickenInFee(bond.lusdAmount), backingRatio);
    }
